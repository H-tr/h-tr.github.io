...
为了简单方便，我们尝试GPT3的时候可以直接去找别人搭好的[playground](https://github.com/shreyashankar/gpt3-sandbox)。将gpt3-sandbox下载下来之后我们选择其中的`gpt.py`来辅助我们的开发。我们假设你现在在linux环境下开发（使用Windows的同学可以尝试WSL, Windows Subsystem Linux），在terminal中输入：
```
mkdir GPTtry
```
...
使用GPT的时候需要OpenAI给的secret key，这个上OpenAI的官网注册账号，然后就能拿到。建立一个GPT_SECRET_KEY.json文件，拿到secret key以后将其储存在json文件中方便我们调用：
```json
{"API_KEY":"your-gpt-secret-key"}
```
用你自己获得的SECRET KEY代替your-gpt-secret-key。然后新建一个文件`GPT3.py`，用任何你喜欢的编辑器打开，在里面加入代码。
...
gpt = GPT(engine="davinci",
          ...
          max_tokens=100)
```
现在我们希望给这个语言模型一些范例，让它能够按照我所希望的样子工作。我们来给这个语言模型一些自然语言，并给它有相同语义的sql语句。我们希望它能够从这些“自然语言-sql语言”对中找到合适的转换逻辑，然后它能够帮我们转换更多更通用的自然语言指令。
```python
...
在OpenAI的网站上，有一个GPT3的playground。在这里我们可以调整参数，从而对与训练语言模型进行微调。微调之后可以直接导出一段代码来设定python程序中语言模型的超参数。我在其中已经尝试了一些调参，得到了一个我个人比较喜欢的结果，也希望大家能够自己玩一玩，得到自己想要的模型。现在我们先建立一个文件
```
touch QAsystem.py
```
...
if __name__ == '__main__':
    chat()
```
其中prompt是传入语言模型的初始对话，它可以让语言模型知道接下来应该按照什么样的模式进行对话和问答。然后我们每次输入的话都应该不断地添加在prompt后面，这样语言模型才能够记住之前有过什么对话，从而不会出现明显的割裂和违和感。
